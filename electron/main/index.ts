          ...
          const promise = fetch(url, {
            ...
            .then(async (response) => {
              ...
              if (response.ok) {
                ...
                logger.info(
                  { arch, category, rawJson: JSON.stringify(json, null, 2) },
...
ipcMain.handle("submit-app", async (event, formData: unknown) => {
  try {
    ...
    logger.info(
      { debFilePath, debFileName: getUUIDFileNameSuggestDeb(debFilePath) },
...
ipcMain.handle("package-app", async (event, formData: unknown) => {
  try {
    const startTime = Date.now();
    logger.info("[Submitter] ============== PACKAGE APP START ==============");
    ...
    if (!packageCategory) return { success: false, message: "分类不能为空" };
    ...
    const sendPackageProgress = (
      ...
      try {
        event.sender.send("package-progress", { step, progress, message });
      } catch (e) {
    ...
    if (iconPath) {
      ...
      if (iconPath.startsWith("http://") || iconPath.startsWith("https://")) {
        ...
        try {
          const response = await fetch(iconPath, { signal: controller.signal });
    ...
    logger.info({ count: screenshots.length }, "[Submitter] Processing screenshots...");
    ...
    for (let i = 0; i < screenshots.length && i < 5; i++) {
      ...
      if (typeof screenshot === "string") {
        if (
          ...
          const timeout = setTimeout(() => controller.abort(), 10000);
          try {
            const response = await fetch(screenshot, { signal: controller.signal });
            clearTimeout(timeout);
            if (response.ok) {
              const buffer = Buffer.from(await response.arrayBuffer());
          ...
          if (base64Data) {
            fs.writeFileSync(screenFilePath, Buffer.from(base64Data, "base64"));
          }
    ...
    try {
      ...
      return { success: false, message: `读取 deb 信息失败: ${(debErr as Error).message}` };
    }
    ...
    logger.info({ debPkgName, debVersion, debArch }, "[Submitter] Deb metadata parsed");

    const packDebFileName = `${debPkgName}_${debVersion}_${debArch}.deb`;
    logger.info({ from: debFilePath, to: path.join(packDir, packDebFileName) }, "[Submitter] Copying deb file...");
    ...
    if (!fs.existsSync(tarOutputPath)) {
      return { success: false, message: "打包后的 tar.gz 文件未找到" };
    }
    ...
    const duration = Date.now() - startTime;
    logger.info(
      { tarOutputPath: tarOutputPath, duration, tempDir: baseTempDir },
    ...
    logger.error("[Submitter] ============== PACKAGE EXCEPTION ==============");
    logger.error(
      {
        errorMessage: (err as Error)?.message,
        errorStack: (err as Error)?.stack,
    ...
    return { success: false, message: (err as Error)?.message || "打包失败" };
  }
});
